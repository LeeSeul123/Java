...
public class BoardDAO extends DAO{
	...
	public List<Board> getMyreco(){
		...
		try {
			conn();
			String sql = "SELECT * FROM recommendation WHERE member_id = ?";
			pstmt = conn.prepareStatement(sql);
	...
	public List<Board> getAllBoard(){
		...
		try {
			conn();
			//원래 게시판 쿼리문
			//String sql = "SELECT * FROM (SELECT ROWNUM, b.* FROM(SELECT * FROM board ORDER BY wr_date) b WHERE rownum <= ?)";
	...
	//상세글 조회2(댓글 불러오기)
	public List<Reply> getReply(int selectNo) {
	...
	//추천하기
	public int setReco(int selectNo) {
		int result = 0;
		...
		try {
			...
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, MemberService.memberInfo.getMemberId());
			pstmt.setInt(2, BoardService.currentPage * BoardInApplication.pageSize + selectNo);
			result = pstmt.executeUpdate();
